    ...
    .get((req, res) => {
        ...
        payLoad.lookFor = queryList.map((item, index) => {
            ...
            if (newValue) {
                ...
                return `CAST("${key}" AS TEXT) ~ '${newValue}'`
            }
        }).filter(item => item)
        
        payLoad.lookForQuery = payLoad.lookFor.join(' AND ')
        payLoad.sql = 'SELECT "ID", "String", "Integer", "Float", to_char("Date", \'YYYY-MM-DD\') AS "Date", "Boolean" FROM data_type WHERE ' + payLoad.lookForQuery + ' ORDER BY "ID" ASC'

        console.log(payLoad);
        dbPostgres.query(payLoad.sql, (err, result) => {
            console.log(result);
            payLoad.data = result.rows
            res.render('./c21/search', {data: payLoad.data, filter})
        })
